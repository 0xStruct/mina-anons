import { verifyMembershipProgram, Bytes64, MerkleProof } from './membership.js';
...
console.time('verify membership (build constraint system)');
let program = await verifyMembershipProgram.analyzeMethods();
console.timeEnd('verify membership (build constraint system)');
console.log(program.verifyMembership.summary());

// compile and prove

console.time('verify membership (compile)');
await verifyMembershipProgram.compile();
console.timeEnd('verify membership (compile)');

console.time('verify membership (prove)');
...
console.timeEnd('verify membership (prove)');

proof.publicOutput.assertTrue('non-membership');
assert(await verifyMembershipProgram.verify(proof), 'proof wrong');
